...
class WindowsViz:
    ...
    def calcCells(self):
        ...
        tabU, tabV = np.array(self.__values[:, 0]), np.array(self.__values[:, 1])
    ...
    def add_drop_point(self, dropPoint):
        ...
        else: self.__dropPoints.append(dropPoint)
...
if __name__ == "__main__":
    ...
    # Events handling
    fig.canvas.mpl_connect('key_press_event', lambda event: evt_on_button_pressed(event, windows))
    fig.canvas.mpl_connect('button_press_event', lambda event: evt_on_mouse_pressed(event, windows))
