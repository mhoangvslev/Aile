...
def charger_objet(obj, limit=0):
    ...
    for line in file.readlines():
        ...
        x, y, z, u, v, w = float(data[0]), float(data[1]), 0, float(data[2]), float(data[3]), 0
...
class WindowsViz:
    ...
    def __init__(self):
        ...
        self.__xs, self.__ys, self.__zs, self.__us, self.__vs, self.__ws, self.__speeds = self.recalibrate()
    ...
    def recalibrate(self):
        x, y, z = self.__coords[:, 0], self.__coords[:, 1], self.__coords[:, 2]
        u, v, w = self.__values[:, 0], self.__values[:, 1], self.__values[:, 2]

        nx, ny, nz = len(x), len(y), len(z)
        xr = np.linspace(x.min(), x.max(), nx)
        yr = np.linspace(y.min(), y.max(), ny)
        zr = np.linspace(z.min(), z.max(), nz)

        xs, ys, zs = np.meshgrid(xr, yr, zr)

        px, py, pz, pu, pv, pw = x.flatten(), y.flatten(), z.flatten(), u.flatten(), v.flatten(), w.flatten()

        us = griddata((px, py, pz), pu, (xs, ys, zs))
        vs = griddata((px, py, pz), pv, (xs, ys, zs))
        ws = griddata((px, py, pz), pw, (xs, ys, zs))

        speeds = np.sqrt(us*us + vs*vs + ws*ws)

        return xs, ys, zs, us, vs, ws, speeds
